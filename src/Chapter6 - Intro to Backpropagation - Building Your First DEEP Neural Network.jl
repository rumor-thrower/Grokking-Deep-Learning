...
function fit_factory(input::Bits, goal::Bool, alpha::Float64)::Function where Bits
	
	function fit(weights::Vector{R}, epochs_left::Int)::Vector{R} where R<:Real
		
		if epochs_left < 1
			return weights
		end
		pred::R = LinearAlgebra.dot(input, weights)
		delta::R = pred - goal
		error::R = delta ^ 2
		weights::Vector{R} -= alpha * input * delta
